...
def zoom_on_cat_face(jpg_path, cat_path, image_size=64, data_augmentation=False):
    ...
    img_scaled = img_pil.transform(img_pil.size, Image.AFFINE, (a, b, c, d, e, f), Image.Resampling.BILINEAR)
    ...
    img_rotated = img_scaled.rotate(sign * rotation, center=new_leye_center, resample=Image.Resampling.BILINEAR)
...
def zoom_on_cat_face_tf(jpg_path, cat_path):
    ...
    '''
    a = tf.py_function(
        func=zoom_on_cat_face,
        inp=[jpg_path, cat_path],
        Tout=tf.uint8
    )
    return a
